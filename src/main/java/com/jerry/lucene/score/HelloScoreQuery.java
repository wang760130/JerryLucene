package com.jerry.lucene.score;
...
import com.jerry.lucene.index.FileIndexHelper;
 ...
 * @date   2016年10月11日
 */
public class HelloScoreQuery {

	public void searchByScoreQuery() {
		try {
			IndexSearcher searcher = FileIndexHelper.getSearcher();
			...
			for(ScoreDoc scoreDoc : tds.scoreDocs) {
				...
				FileIndexHelper.print(scoreDoc, document);
			}
			searcher.close();
		} catch (IOException e) {
			e.printStackTrace();
